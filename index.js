...
app.post('/update', function(req, res) {
  ...
  if (req.body['id'] === undefined ||
    ...
    res.statusCode = 400;
    return res.json(error_builder('id must be present', res.statusCode));
  }

  if (req.body['time'] && typeof(req.body['time']) != 'number') {
    res.statusCode = 400;
    return res.json(error_builder('time must be a number', res.statusCode));
  }

  if (req.body['weight'] && typeof(req.body['weight']) != 'number') {
    res.statusCode = 400;
    return res.json(error_builder('weight must be a number', res.statusCode));
  }
  ...
  authorize_user(req.body['username'], req.body['auth_token'], (success) => {
    ...
    redisClient.hget(exercises_key(req.body['username']),
      req.body['id'],
      (err, obj) => {
        var jobj;
        if (!obj) {
          res.statusCode = 400;
          return res.json(error_builder('bad id', res.statusCode));
        }
        if (err) {
          res.statusCode = 500;
